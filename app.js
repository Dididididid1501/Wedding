(function() {
    // --- Инициализация SupabaseClient (без конфликта имён) ---
    ...
    function updateProgressFromTasks() {
        ...
        setProgress(percent);
        if (supabaseClient) {
            supabaseClient.from('users').update({ progress_percent: percent }).eq('user_id', userId).catch(e => console.error);
        }
    }
    ...
    async function loadUserData() {
        ...
        try {
            ...
            if (!user) {
                await supabaseClient.from('users').insert([{ user_id: userId, full_name: userName, wedding_date: targetDate.toISOString() }]);
            } else {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Если включена SES или CSP, добавим временный мета-тег для разрешения unsafe-eval
        if (typeof ses !== 'undefined' || document.querySelector('meta[http-equiv="Content-Security-Policy"]')) {
            console.warn('Обнаружена CSP или SES, пробуем добавить unsafe-eval');
            ...
            if (!meta) {
                ...
                console.log('Добавлен CSP с unsafe-eval. Перезагрузите страницу.');
            }
        }
